...
FinalInventory InventoryManager::SettleInventory(
    ...
    uint16_t base_weight) 
{
    ...
    for (int i = 0; i < FRIGERATOR_HISTORY_INFO_SIZE; ++i) {
        if (history.weightTimestamp[i] > 0) { 
                    int16_t dw = static_cast<int16_t>(history.weight[i]) - static_cast<int16_t>(last_w);
                    if (std::abs(dw) > 10) { 
                        weight_events.push_back({history.weightTimestamp[i], dw, false});
                    }
    ...
    for (uint8_t i = 0; i < dyn_res.fruitCount; ++i) {
        ...
        if (matched_weight > 0) {
            ...
            new_fruit.estimated_weight = static_cast<uint32_t>(matched_weight); // 绑定单体重量到水果实例
    ...
    for (uint8_t i = 0; i < static_res.fruitCount; ++i) {
        static_counts[static_res.fruits[i].fruitType]++;
        UpdateFreshnessFromStatic(static_res.fruits[i].fruitType, static_res);
    }
    ...
    for (const auto& pair : static_counts) {
        ...
        while (current < target) {
            ...
            // 静态强行补齐的记录，无法得知准确单体重量，设为999或按均值处理
            TrackedFruit nf; 
            nf.type = type; 
            nf.put_in_timestamp = static_res.timestamp;
            nf.uid = GenerateUID(type, static_res.timestamp); 
            nf.freshness = FreshnessLevel::FRESH; 
            nf.estimated_weight = 999; 
...
void InventoryManager::UpdateFreshnessFromStatic(FruitType type, const StaticRecognitionResult& static_res) {
    ...
    for (uint8_t i = 0; i < static_res.fruitCount; ++i) {
        if (static_res.fruits[i].fruitType == type) {
            detected_freshness.push_back(static_res.fruits[i].freshness);
        }
    }
    std::sort(stock_[type].begin(), stock_[type].end(), [](const TrackedFruit& a, const TrackedFruit& b) { return a.put_in_timestamp < b.put_in_timestamp; });
...
FinalInventory InventoryManager::GenerateFinalStruct() {
    FinalInventory final_res;
    final_res.fruitCount = 0;

    for (const auto& pair : stock_) {
        for (const auto& fruit : pair.second) {
            if (final_res.fruitCount < MAX_STATIC_FRUIT_COUNT) { 
                // 赋值基本信息
                final_res.fruits[final_res.fruitCount].fruitInfo.fruitType = fruit.type;
                final_res.fruits[final_res.fruitCount].fruitInfo.freshness = fruit.freshness;
                final_res.fruits[final_res.fruitCount].fruitInfo.locationX = 0; // 缺失坐标可填0，或视需求从静态结果继承
                final_res.fruits[final_res.fruitCount].fruitInfo.locationY = 0;
                
                // 赋值单体重量
                final_res.fruits[final_res.fruitCount].weight = fruit.estimated_weight; 

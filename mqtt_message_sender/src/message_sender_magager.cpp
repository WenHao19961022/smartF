...
std::string SerializeMqttMessageToJson(const MqttMessageStruct& message) {
    ...
    const char* eventType = "snapshot";
    if (message.eventType == MqttEventType::Startup) eventType = "startup";
    if (message.eventType == MqttEventType::RecognitionError) eventType = "recognition_error";
